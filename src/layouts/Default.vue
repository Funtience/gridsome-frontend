<template>
  <div>
    ...
    >
      ...
      <a
        :href="'https://github.com/' + general.username"
        class="btn"
        target="_blank"
        >GitHub主页</a
      >
        ...
        <el-row>
          ...
          >
            <el-row>
              ...
              <el-col :span="14" style="padding-left: 20px">
                <el-slider
                  @change="changeTime"
                  :format-tooltip="$utils.formatTime"
                  :max="music.maxTime"
              ...
              >
                {{ $utils.formatTime(music.currentTime) }}/{{
                  $utils.formatTime(music.maxTime)
                }}
              </el-col>
            </el-row>
          ...
          <el-col :span="4" style="text-align: right;">
            <div style="font-size: 20px;color:#606266;margin-top: 5px">
              <b>{{ general.username }}</b>
            </div>
            <div style="color:#606266;">
              <i class="el-icon-location"></i>&nbsp;{{
                general.location ? general.location : '未填写地址'
              }}
              <br />
            </div>
          </el-col>
          <el-col :span="2" style="text-align: center;">
            ...
            <el-popover
              ...
              trigger="hover"
            >
              <i class="el-icon-star-on"></i>&emsp;{{ general.username }}
              <br />
              <i class="el-icon-location"></i>&emsp;{{ general.location }}
              <br />
              <img
                :src="`${GRIDSOME_API_URL}${general.avatar.url}`"
...
export default {
  ...
  mounted() {
    ...
    for (let i = 0; i < 12; i++) {
      let temp = {}
      let left = this.$utils.randomInt(10, width - 310)
      if (left > width / 2 - 150) {
        left += 300
      }
      temp['left'] = left
      temp['top'] = this.$utils.randomInt(20, 300)
  ...
  methods: {
    selectTopbar(index) {
      ...
      switch (index) {
        case '#githubHome':
          window.open('https://github.com/' + this.general.username)
          break
        case '#blog':
          this.$message({
            message: '博主没有其他博客',
    ...
    moveIcon(index) {
      let width = window.innerWidth
      this.randomIcon[index]['top'] = this.$utils.randomInt(20, 300)
      let left = this.$utils.randomInt(10, width - 310)
      if (left > width / 2 - 150) {
        left += 300
      }
